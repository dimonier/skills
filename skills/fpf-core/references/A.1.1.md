...
5. **Maintenance and change stay separate.**
  - **Work:** `Engineer-4 : U.System` performs `ControllerCoherenceWork-22 : U.Work` under `ControllerEngineerAssignment-7`. That assignment obtains, names Engineer-4 as holder, and covers the Work. The Work has the complete A.15.1/F.6 basis used in this case and enacts `ControllerAlignmentMethod-2`.
...
6. **Evaluation and result stay separate.**
  - **Evaluation Work:** `Evaluator-2 : U.System` performs `CoherenceEvaluationWork-23 : U.Work` under `CoherenceEvaluatorAssignment-5`. That assignment obtains, names Evaluator-2 as holder, and covers the Work. The Work has the complete A.15.1/F.6 basis used in this case and enacts `CoherenceEvaluationMethod-4`. State any needed operation application through its A.6.1 binding.
...
1. **Method and Work.** `Architect-9 : U.System` performs dated `ContextMappingWork-14 : U.Work` under `ArchitectureAssignment-6`. That assignment obtains, names Architect-9 as holder, and covers the Work. The Work has the complete A.15.1/F.6 basis used in this case and enacts `ContextMappingMethod-3 : U.Method`. The repeatable Method and this dated Work remain different objects.
...
11. The structure is omitted when its joint organization changes no receiving decision. The reader can name both the admissible action and the nearest overread; otherwise the reader stops at the direct relation or subject pattern.
